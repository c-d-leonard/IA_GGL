...
e_rms_Bl_a 		= 	0.18 # rms ellipticity of sample a, Blazek method
e_rms_Bl_b		=	0.18 # rms ellipticity of sample b, Blazek method
...
n_l 			= 	2.9*10**2 # The number of lenses in the lens sample per square DEGREE
Area_l 			=	3000 # Area associated with the lens sample in square DEGREES
fsky			=   Area_l / 41253. # Assumes the lens area is the limiting factor
n_s 			=	26 # The number density of sources in the sample per square ARCMINUTE
S_to_N 			= 	500. # THIS IS A MADE UP NUMBER THAT IS BIG TO MAKE ERMS DOMINATE FOR NOW. # The signal to noise of the lensing measurement (?) - necessary for estimating sigma_e
...
N_shapes		= 	2.8*10**8 # Number of galaxies in the shape sample.
N_LRG			=	4. * 10**6 # Numbr of galaxies in the LRG sample.
...
alpha_sys 	=	alpha_fid * (1. - np.asarray([0.1, 0.2, 0.3])) 
z0_sys		= 	z0_fid * ( 1. - np.asarray([0.1, 0.2, 0.3])) 
beta_sys 	=	beta_fid * ( 1. - np.asarray([0.1, 0.2, 0.3])) 
dNdzpar_sys	= 	[alpha_sys, z0_sys, beta_sys] # A 2d list, for passing this around.

pztype		=	'Gaussian'
#zpts		=	1000  # Number of points in the z vector at which we are evaluating dNdz
sigz_fid	=	0.05  # The photometric redshift error given a Gaussian photo_z model
...
zeff 	= 	0.8  # The effective redshift of the lens sample
...
zphmin	=	zeff
zphmax	=	5.0
delta_z	=	0.1  # THIS IS ARBITRARY AT THE MOMENT # The width of the redshift slice which begins at the lens and ends at the top of sample a
zmin_dndz = zsmin
...
kpts_wgg = 10000
kpts_wgp = 2000
bs_Bl = 1. # Source sample 
bs_shapes = 1. # Source sample
bd_Bl = 2.07 # Lens sample
bd_shapes = 2.07 # Lens sample
