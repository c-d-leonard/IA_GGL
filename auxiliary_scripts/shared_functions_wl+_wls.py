...
def wgp_full(rp_c, Ai, ah, q11, q12, q13, q21, q22, q23, q31, q32, q33, savefile_1h, savefile_2h, plotfile):
	...
	#(rp_cen, wgp_1h) = np.loadtxt('./txtfiles/wgp_1halo_LRG.txt', unpack=True)
	#print "DONE WITH WGP_1H, ON TO WGP 2H"
	
	wgp_2h = wgp_2halo(rp_c, rp_cents_, bd, Ai, savefile_2h)
	#print "DONE WITH 2 HALO WGP"
	#(rp_cen, wgp_2h) = np.loadtxt('./txtfiles/wgp_2halo_'+str(pa.kpts_wgp)+'pts_LRG.txt', unpack=True)
...
def wgg_1halo_Four(rp_cents_, fsat, ng, savefile):
	...
	Pk = get_Pkgg_1halo(rvec_NFW, kvec_FT, fsat, ng) # Gets the 1halo galaxy power spectrum including c-s and s-s terms. Pass rvec because need to get rho_NFW in here.
...
def get_Pkgg_1halo(rvec_nfw, kvec_ft, fsat, ng):
	""" Returns the 1halo galaxy power spectrum with c-s and s-s terms."""
	
	print "THIS DOESN'T DEAL WITH PROPER TERMS FOR THE LENS-SOURCE CASE - NEED TO DERIVE THIS."
...
def wgg_full(rp_c, fsat, ng, bd, bs, savefile_1h, savefile_2h, plotfile):
	""" Combine 1 and 2 halo terms of wgg """
	
	print "GETTING 1HALO WGG"
	wgg_1h = wgg_1halo_Four(rp_c, fsat, ng, savefile_1h)
	...
	#(rp_cen, wgg_2h) = np.loadtxt('./txtfiles/wgg_2halo_'+str(pa.kpts_wgg)+'pts_LRG.txt', unpack=True)
