...
\begin{equation}
P_{1h}(k,z) \frac{M_{\rm vir}^2}{\bar{\rho}_M^2} n_h u(k,M_{\rm vir})^2 = \frac{M_{\rm vir}}{\Omega_M(z) \rho_c} u(k,M_{\rm vir})^2
\label{P1h_onemass}
\end{equation}
where $n_h$ is the volume number density of halos. A choice of $M_{\rm vir} = 4 \times 10^{12} M_{\odot}$ given a 1-halo term that roughly matches $w_{gg}$ from Singh et al. 2014. {\it However, the halo mass measured for the sample is $13.18 \pm 0.05$. Picking this mass gives $w_{gg}$ that does not match at all.}
