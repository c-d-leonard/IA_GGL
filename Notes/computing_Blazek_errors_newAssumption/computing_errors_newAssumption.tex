...
\paragraph{A note on computing $\sigma(\tilde{\Delta \Sigma}^i)$}: 
The variance of $\tilde{\Delta\Sigma}$ is given by:
\begin{equation}
\sigma^2(\tilde{\Delta\Sigma}) = \frac{\sum\limits_{ls} \tilde{w}^2 \tilde{\Sigma_c}^2 \sigma^2_\gamma}{\sum\limits_{ls} \tilde{w}^2}
\label{varDS}
\end{equation}
where sumation indices have been neglected for clarity and $\sigma^2_\gamma$ is equal to the rms ellipticity and measurement error added in quadrature. Since we know that weights are given as $\tilde{w} = (\tilde{\Sigma}_c \sigma_\gamma)^{-2}$, we have:
\begin{equation}
\sigma^2(\tilde{\Delta\Sigma}) = \frac{\sum\limits_{ls} \tilde{w}}{\sum\limits_{ls} \tilde{w}^2} = \frac{1}{\sum\limits_{ls} \tilde{w}} = \frac{1}{\sum\limits_{ls} (\tilde{\Sigma}_c \sigma_\gamma)^{-2}}
\label{varDS_2}
\end{equation}
We assume that the measurement noise, which depends on the particular lens-source pair in princple, can be pulled out of the sum along with the rms ellipticity to get:
\begin{equation}
\sigma^2(\tilde{\Delta\Sigma}) = \frac{\sigma^2_\gamma}{\sum\limits_{ls} \tilde{\Sigma}_c^{-2}}.
\label{varDS_3}
\end{equation}
The problem is then reduced to finding the correct expression for the denominator of the above equation, which is given by:
\begin{equation}
\sum\limits_{ls} (\tilde{\Sigma}_c)^{-2} = N_l N_s^i \frac{\int_{z_{\rm cut_l}}^{z_{\rm cut_h}} dz_p (\tilde{\Sigma}_c)(z_p, z_l)^{-2} \int_{z_{\rm min}}^{z_{\rm max}} dz_s \frac{dN}{dz_s}p(z_s, z_p)}{ \int_{z_{\rm min}}^{z_{\rm max}} dz_p \int_{z_{\rm min}}^{z_{\rm max}} dz_s \frac{dN}{dz_s}p(z_s, z_p)}.
\label{sum_varDS}
\end{equation}
where $N_l$ is number of lenses and $N_s^i$ is the number of sources in projected radial separation bin $i$.

The above only accounts for `random' sources; we need to also account for excess. {\it However, the above cannot even be consistently applied in our current setup, because we only have the surface density for all sources, not just random, meaning we can only compute $N_s^i$ for (excess + random). This means that even in the random-only case, we don't have an appropriate normalization. We could normalize the integral correctly if we were integrating over the full redshift distribution of sources, but we don't have this information for (excess + random) sources. This is an outstanding issue.}

There are several quantities for which we must make some reasonable calculation in a fiducial case: $c_z$, ($B(r_p)-1+F$), $\langle \tilde{\Sigma}_c\rangle_{\rm IA}(r_p)$, $\widetilde{\Delta\Sigma}(r_p)$, and $\bar{\gamma}^{\rm IA}(r_p)$.
...
Given the NFW mass profile of equation \ref{nfw}, we can compute its Fourier transform, $u(k, M, z)$:
\begin{equation}
u(k, M) = \int_0^{R_{\rm vir}} dr \frac{4 \pi r^2}{M}\frac{\sin(kr)}{kr}\rho(r, M).
\label{u}
\end{equation}

We must account for both the `central-satelite' term and the `satelite-satelite' term of the 2-point spectrum. The 1-halo power spectrum which accounts for both of these terms is (taken from D. Grin, Thesis) is:
\begin{equation}
P_{gg}^{1h}(k) = \frac{n_H}{n_g^2}\left[\langle N_c N_s \rangle u + \langle N_s^2 \rangle u^2\right]
\label{1hpgg}
\end{equation}
where $n_H$ is the volume density of halos at mass $M$, and $n_g$ is the volume density of galaxies in the survey. $\langle N_c N_s \rangle$ is the variance in the number of central-satelite pairs in a halo of mass $M$, while $\langle N_s^2 \rangle$ is the number of satelite-satelite pairs. (I have specified to a single average mass $M$ here.) 

For Poisson statistics, we have that $\langle N^2 \rangle = \langle N \rangle^2$. However, there may be some deviation from Poisson statistics for small mass halos, which we encapsulate by introducing a parameter $\alpha$ such that $\langle N^2 \rangle = \alpha^2\langle N \rangle^2$. $\alpha$ is given by:
\begin{equation}
\alpha = \log\left(\frac{M}{10^{11}M_\odot h^{-1}}\right)^{0.5}
\label{alpha}
\end{equation}
if $M$ is less than $10^{11}$ in units of $M_\odot / h$, and $\alpha=1$ otherwise. We then use this relationship to get the required variances in equation \ref{1hpgg} in terms of the mean number of satelite and central galaxies in a halo of mass $M$. We take these to be given respectively by:
\begin{equation}
\langle N_s \rangle = \left(\frac{M}{M_1}\right)^a {\rm exp}\left(\frac{-M_{\rm cut}}{M}\right)
\label{Nsmean}
\end{equation}
from Zheng 2007 and
\begin{equation}
\langle N_c \rangle = \frac{1}{2}\left[1 + {\rm erf}\left(\frac{\log{M}-\log{M_{\rm min}}}{\sigma_{\log M}}\right)\right].
\label{Ncmean}
\end{equation}

Then, given $P_{gg}^{1h}(k)$, we Fourier-transform and project to get $w_{ls}^{1h}(r_p)$:
\begin{equation}
