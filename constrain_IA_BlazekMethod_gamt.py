...
def get_boost(rp_cents_, sample):
	...
	propfact = np.loadtxt('./txtfiles/boosts/Boost_'+str(sample)+'_gamt_survey='+str(SURVEY)+'_deltaz='+str(pa.delta_z)+'.txt')
...
def get_fred(photoz_samp):
	...
	for zi in range(0, len(zL)):
		...
		elif (photoz_samp =='assocBl+B'):
			if (pa.delta_z<zL[zi]):
				zphmin = zL[zi] - pa.delta_z
			else:
				zphmin = 0.
			zph = np.linspace(zphmin, pa.zphmax, 500)
		elif (photoz_samp =='full'):
...
def gamma_fid(rp):
	...
	f_red = get_fred('assocBl+B')
	print "f_red=", f_red
	fred_save = [0]
	fred_save[0] = f_red
	np.savetxt('./txtfiles/f_red/f_red_gamt_'+SURVEY+'_deltaz='+str(pa.delta_z)+'_rlim='+str(pa.mlim)+'_LSSTpzonly.txt', fred_save)
	f_red = np.loadtxt('./txtfiles/f_red/f_red_gamt_'+SURVEY+'_deltaz='+str(pa.delta_z)+'_rlim='+str(pa.mlim)+'_LSSTpzonly.txt')
...
np.savetxt('./txtfiles/StoN/StoNstat_Blazek_'+SURVEY+'_deltaz='+str(pa.delta_z)+'_gamtBl_rlim='+str(pa.mlim)+'_SNonly_LSSTpz.txt', StoNstat_save)

# Save the statistical + sysB S-to-N   
StoNstat_sysB_save = [StoN_stat_sysB]
np.savetxt('./txtfiles/StoN/StoN_stat_sysB_Blazek_'+SURVEY+'_deltaz='+str(pa.delta_z)+'_gamtBl_rlim='+str(pa.mlim)+'_SNonly_LSSTpz.txt', StoNstat_sysB_save)
