...
def N_of_zph(z_a_def_s, z_b_def_s, z_a_norm_s, z_b_norm_s, z_a_def_ph, z_b_def_ph, z_a_norm_ph, z_b_norm_ph, dNdzpar, pzpar, dNdztype, pztype):
	...
	(z, dNdZ) = setup.get_NofZ_unnormed(dNdzpar, dNdztype, z_a_def_s, z_b_def_s, pa.zpts)
	(z_norm, dNdZ_norm) = setup.get_NofZ_unnormed(dNdzpar, dNdztype, z_a_norm_s, z_b_norm_s, pa.zpts)
...
def get_bSigW(z_p_min, z_p_max, e_rms, pzpar, pztype):
	...
	for zi in range(0,len(z_p_vec)):
		...
		if (pztype == 'Gaussian'):
			...
			zsvec = np.random.normal(z_p_vec[zi], pzpar[0], 10**7)
...
def get_cz(z_l, z_ph_min, z_ph_max, erms, rp_bins_, rp_bins_c, dNdzpar, pzpar, dNdztype, pztype):
	...
	#print "in getcz, dNdzpar=", dNdzpar
	#print "in getcz, pzpar=", pzpar
...
def get_gammaIA_cov(rp_bins, rp_bins_c, dNdz_sys, pz_sys):
	...
	if pa.run_quants == True:
		...
		save_F = np.column_stack((rp_bins_c, F_a_fid, F_b_fid, F_a_sp, F_b_sp))
		np.savetxt('./txtfiles/F_afid_bfid_asp_bsp.txt', save_F)
		...
		save_SigIA = np.column_stack((rp_bins_c, Sig_IA_a_fid, Sig_IA_b_fid, Sig_IA_a_sp, Sig_IA_b_sp))
		np.savetxt('./txtfiles/Sig_IA_afid_bfid_asp_bsp.txt', save_SigIA)
		...
		save_cz = np.column_stack(([cz_a_fid], [cz_b_fid], [cz_a_sp], [cz_b_sp]))
		np.savetxt('./txtfiles/cz_afid_bfid_asp_bsp.txt', save_cz)
	...
	if pa.run_quants==False :
		...
		(rp_bins_c, F_a_fid, F_b_fid, F_a_sp, F_b_sp) = np.loadtxt('./txtfiles/F_afid_bfid_asp_bsp.txt', unpack=True)
		(rp_bins_c, Sig_IA_a_fid, Sig_IA_b_fid, Sig_IA_a_sp, Sig_IA_b_sp) = np.loadtxt('./txtfiles/Sig_IA_afid_bfid_asp_bsp.txt', unpack=True)
		(cz_a_fid, cz_b_fid, cz_a_sp, cz_b_sp) = np.loadtxt('./txtfiles/cz_afid_bfid_asp_bsp.txt', unpack=True)
	...
	plt.figure()
	plt.loglog(rp_bins_c, DeltaSig_est_a, 'go', label='a fid')
	plt.hold(True)
	plt.loglog(rp_bins_c, DeltaSig_est_b, 'mo', label='b fid')
	plt.legend()
	plt.xlabel('$r_p$')
	plt.ylabel('$\Sigma_c$')
	plt.xlim(0.04, 20)
	plt.savefig('./plots/DeltaEst_%sys='+str((pa.dNdzpar_fid[0]- dNdz_sys[0]) / pa.dNdzpar_fid[0])+'_parmod.pdf')
	plt.close()
	...
	np.savetxt('./txtfiles/frac_StatError_Blazek_LRG-shapes_mod_dNdz_pz_pars.txt', save_variance)
	...
	plt.figure()
	plt.loglog(rp_bins_c, gamma_from_sp, 'g+', label='a fid')
	plt.legend()
	plt.xlabel('$r_p$')
	plt.ylabel('$\Sigma_c$')
	plt.xlim(0.04, 20)
	plt.savefig('./plots/gamma_sp_%sys='+str((pa.dNdzpar_fid[0]- dNdz_sys[0]) / pa.dNdzpar_fid[0])+'_parmod.pdf')
	plt.close()
	...
	plt.figure()
	plt.loglog(rp_bins_c, sigma_sp, 'g+', label='a fid')
	plt.legend()
	plt.xlabel('$r_p$')
	plt.xlim(0.04, 20)
	plt.savefig('./plots/sigma_sp_%sys='+str((pa.dNdzpar_fid[0]- dNdz_sys[0]) / pa.dNdzpar_fid[0])+'_parmod.pdf')
	plt.close()
	
	plt.figure()
	plt.loglog(rp_bins_c, g_IA_fid, 'g+', label='a fid')
	plt.legend()
	plt.xlabel('$r_p$')
	plt.xlim(0.04, 20)
	plt.savefig('./plots/gIA_%sys='+str((pa.dNdzpar_fid[0]- dNdz_sys[0]) / pa.dNdzpar_fid[0])+'_parmod.pdf')
	plt.close()
	...
	plt.ylabel('$\sigma(\gamma_{IA})$')
	plt.xlim(0.04, 20)
	plt.savefig('./plots/frac_sys_errors_Blazek_LRG-shapes_mod_dNdz_pz_pars_%sys='+str((pa.dNdzpar_fid[0]- dNdz_sys[0]) / pa.dNdzpar_fid[0])+'.pdf')
	plt.close()
	...
	plt.ylabel('$\sigma(\gamma_{IA})$')
	plt.xlim(0.04, 20)
	plt.savefig('./plots/sys_errors_Blazek_LRG-shapes_'+str((pa.dNdzpar_fid[0]- dNdz_sys[0]) / pa.dNdzpar_fid[0])+'.pdf')
	plt.close()

	# Save the fractional sys error
	save_variance = np.column_stack((rp_bins_c, np.sqrt(np.diag(gamma_IA_sys_cov)) / g_IA_fid))
	np.savetxt('./txtfiles/frac_SysError_Blazek_LRG-shapes_mod_dNdz_pz_pars_'+str((pa.dNdzpar_fid[0]- dNdz_sys[0]) / pa.dNdzpar_fid[0])+'.txt', save_variance)
	...
	np.savetxt('./txtfiles/frac_totalError_Blazek_LRG-shapes_mod_dNdz_pz_pars_'+str((pa.dNdzpar_fid[0]- dNdz_sys[0]) / pa.dNdzpar_fid[0])+'.txt', save_variance)
	...
	plt.ylabel('$\sigma(\gamma_{IA})$')
	plt.xlim(0.04, 20)
	plt.savefig('./plots/absolute_error_Blazek_LRG-shapes_mod_dNdz_pz_pars_'+str((pa.dNdzpar_fid[0]- dNdz_sys[0]) / pa.dNdzpar_fid[0])+'.pdf')
	plt.close()
...
def gamma_fid(rp):
	...
	wgg_rp = ws.wgg_full(rp, pa.fsat_LRG, pa.fsky, pa.bd_Bl, pa.bs_Bl, './txtfiles/wgg_1h_Blazek_LRG-shapes_7bins_NsatHOD.txt', './txtfiles/wgg_2h_Blazek_LRG_shapes_7bins.txt', './plots/wgg_full_Blazek_LRG-shapes_7bins_NsatHOD.pdf')
	...
	plt.savefig('./plots/gammaIA_Blazek_LRG-shapes_7bins_NsatHOD.pdf')
	plt.close()
...
def plot_variance(cov_1, fidvalues_1, bin_centers):
	...
	plt.tight_layout()
	plt.savefig('./plots/stat+sys_log_BlazekMethod_LRG-shapes_mod_dNdz_pz_pars_'+str((pa.dNdzpar_fid[0]- pa.dNdzpar_sys[i][0]) / pa.dNdzpar_fid[0])+'.pdf')
	plt.close()
	...
	fig_sub.tick_params(axis='both', which='minor', labelsize=12)
	plt.tight_layout()
	plt.savefig('./plots/stat+sys_notloBlazekMethod_LRG-shapes_mod_dNdz_pz_pars_7bins_'+str((pa.dNdzpar_fid[0]- pa.dNdzpar_sys[i][0]) / pa.dNdzpar_fid[0])+'.pdf')
	plt.close()
...
for i in range(0,pa.num_sys):
	...
	#print "dNdzpar_sys=", pa.dNdzpar_sys[i], "pzpar_sys=", pa.pzpar_sys[i]
	
	# Get the statistical error on gammaIA
	(Cov_gIA, fid_gIA) = get_gammaIA_cov(rp_bins, rp_cent, pa.dNdzpar_sys[i], pa.pzpar_sys[i])
