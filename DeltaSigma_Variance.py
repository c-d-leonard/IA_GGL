...
def getwbar():
	...
	save=[0]
	save[0]= wbar
	np.savetxt('./txtfiles/wbar_extl'+endfilename+'_sample='+SAMPLE+'_deltaz='+str(pa.delta_z)+'.txt', save)
...
def get_ns_partial(zL):
	...
	#frac = sum_weights(SAMPLE, 'nocut', Redges, pa.dNdzpar_fid, pa.pzpar_fid, pa.dNdztype, pa.pztype) / sum_weights('full', 'nocut', Redges, pa.dNdzpar_fid, pa.pzpar_fid, pa.dNdztype, pa.pztype) 
	...
	print "fraction of neff in the sample=", averaged_ns_samp / ns_tot
...
"""def get_ns_partial():
	...
	#frac = np.asarray(sum_weights(pa.zeff, pa.zsmin, pa.zsmax, pa.zsmin, pa.zsmax, pa.zeff, pa.zeff+pa.delta_z, pa.zphmin, pa.zphmax, pa.e_rms_Bl_a, Redges, Rcentres, pa.dNdzpar_fid, pa.pzpar_fid, pa.dNdztype, pa.pztype))[0] / np.asarray(sum_weights(pa.zeff, pa.zsmin, pa.zsmax, pa.zsmin, pa.zsmax, pa.zphmin, pa.zphmax, pa.zphmin, pa.zphmax, pa.e_rms_Bl_a, Redges, Rcentres, pa.dNdzpar_fid, pa.pzpar_fid, pa.dNdztype, pa.pztype))[0]
	
	frac = sum_weights('A', 'nocut', Redges, pa.dNdzpar_fid, pa.pzpar_fid, pa.dNdztype, pa.pztype) / sum_weights('full', 'nocut', Redges, pa.dNdzpar_fid, pa.pzpar_fid, pa.dNdztype, pa.pztype)
...
def doints_Pgk():
	...
	np.savetxt('./txtfiles/Pgkterm_DeltaSig_extl_'+endfilename+'_sample='+SAMPLE+'_deltaz='+str(pa.delta_z)+'.txt', int_gk**2 )
...
def doints_Pkk():
	...
	np.savetxt('./txtfiles/Pkkterm_DeltaSig_extl_'+endfilename+'_sample='+SAMPLE+'_deltaz='+str(pa.delta_z)+'.txt', int_kk)
...
def doints_PggPkk():
	...
	np.savetxt(folderpath+outputfolder+'/PggPkkterm_DeltaSig_extl_'+endfilename+'_sample='+SAMPLE+'_deltaz='+str(pa.delta_z)+'.txt', int_kkgg)
...
def doconstint():
	...
	print "gam=", gam
	print "nl=", nl
	print "chiSint=", chiSint_zl
	
	print "save[0]=", save[0]
	
	np.savetxt('./txtfiles/const_DeltaSig_extl_'+endfilename+'_sample='+SAMPLE+'_deltaz='+str(pa.delta_z)+'.txt', save)
